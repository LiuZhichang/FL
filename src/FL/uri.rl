...
# See RFC 3986: http://www.ietf.org/rfc/rfc3986.txt
		...
		action save_scheme
		{
			curi->setScheme(std::string(mark, fpc - mark));
			mark = NULL;
		}
		...
		action save_userinfo
		{
			if(mark) {
				...
				curi->setUserinfo(std::string(mark, fpc - mark));
			}
			mark = NULL;
		}
		action save_host
		{
			if (mark != NULL) {
				//std::cout << std::string(mark, fpc - mark) << std::endl;
				curi->setHost(std::string(mark, fpc - mark));
			}
		}
		...
		{
			//std::cout << std::string(mark, fpc - mark) << std::endl;
			curi->setPath(std::string(mark, fpc - mark));
			mark = NULL;
		}
...
# add (any -- ascii) support chinese
		...
		{
			//std::cout << std::string(mark, fpc - mark) << std::endl;
			curi->setQuery(std::string(mark, fpc - mark));
			mark = NULL;
		}
		action save_fragment
		{
			//std::cout << std::string(mark, fpc - mark) << std::endl;
			curi->setFragment(std::string(mark, fpc - mark));
			mark = NULL;
		}
...
main := URI_reference;
	...
	std::ostream& operator << (std::ostream& os, Uri::ptr uri) {
		uri->dump(os);
		return os;
	}
