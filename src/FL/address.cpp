...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if(error) {
        FL_LOG_ERROR(FL_SYS_LOG()) << "Address::Lookup getaddress(" << host << ", "
                                   << family << ", " << type << ") error = " << error << " error info [" << strerror(errno) << "]";
        return false;
    }
...
bool Address::GetInterfaceAddress(std::multimap<std::string
    ...
    if(getifaddrs(&results) != 0) {
        FL_LOG_ERROR(FL_SYS_LOG()) << "Address::GetInterfaceAddress getifaddrs "
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    if(error) {
        FL_LOG_ERROR(FL_SYS_LOG()) << "IPAddres::Create(" << address
                                   << ", " << port << "( error = " << error
                                   << " errno = " << errno << " error info [" << strerror(errno) << "]";
        return nullptr;
    }
...
IPAddress::ptr IPv4Address::broadcastAddress(uint32_t prefix_len) {
    ...
    addr.sin_addr.s_addr |= byteswapOnLittleEndian(
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if(res <= 0) {
        FL_LOG_ERROR(FL_SYS_LOG()) << "IPv4Address::Create(" << address << ", "
                                   << port << ") result = " << res << " errno = " << errno
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if(res <= 0) {
        FL_LOG_ERROR(FL_SYS_LOG()) << "IPv6Address::Create(" << address << ", "
                                   << port << ") result = " << res << " errno = " << errno
                                   << "errno info [" << strerror(errno) << "]";
        return nullptr;
    }
...
const sockaddr* UnkonwAddress::getAddr() const {
    return &m_addr;
}
